...
from src.indexer import (load_faiss_index, load_bm25_index,
                         dense_search_faiss, get_embedder)
...
def bm25_search(bm25, meta_list: List[Dict],
    ...
    for idx in top_idx:
        ...
        m = meta_list[idx]
        chunks.append(RetrievedChunk(
            text=m["text"], source=m["source"], page=m["page"],
            chunk_id=m["chunk_id"], parent_id=m["parent_id"],
            score=float(scores[idx]), method="bm25"
        ))
    return chunks


def dense_search(collection: dict, query: str,
                 top_k: int = DENSE_TOP_K) -> List[RetrievedChunk]:
    results = dense_search_faiss(collection["index"],
                                 collection["meta"], query, top_k)
    return [RetrievedChunk(
        text=r["text"], source=r["source"], page=r["page"],
        chunk_id=r["chunk_id"], parent_id=r["parent_id"],
        score=r["score"], method="dense"
    ) for r in results]
...
def reciprocal_rank_fusion(dense_results: List[RetrievedChunk],
    ...
    for cid in sorted_ids:
        ...
        merged.append(RetrievedChunk(
            ...
            score=rrf_scores[cid], method="hybrid"
        ))
    return merged
...
def retrieve(query: str, collection: dict, bm25, meta_list: List[Dict],
             use_rerank: bool = True) -> List[RetrievedChunk]:
    dense_results = dense_search(collection, query)
    bm25_results  = bm25_search(bm25, meta_list, query)
    merged        = reciprocal_rank_fusion(dense_results, bm25_results)
